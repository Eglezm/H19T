  ...
  return (
    <Card>
          ...
          <thead>
            <tr>
              ...
              {pares.map((par, h) => {
                ...
                return <th key={h} style={{ padding:"4px 4px", color:D.textDim, fontWeight:600, minWidth:28, background:ts?ts.bg:"transparent" }}>{h+1}</th>;
              })}
            ...
            <tr>
              ...
              {pares.map((par, h) => {
                const ts = teeStyle(teeColor(torneo.campo, h));
                return <td key={h} style={{ textAlign:"center", padding:"2px 4px", color:ts?ts.fg:D.textDim, fontSize:10, background:ts?ts.bg:"transparent" }}>{par}</td>;
              })}
      ...
      {CAMPOS[torneo.campo]?.nueveHoyos && pares.length===18 && (
        <div style={{ display:"flex", gap:14, justifyContent:"center", marginTop:10, fontSize:10, color:D.textSub }}>
