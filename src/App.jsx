      ...
      {rows.map((u, pos) => (
        <div key={u.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:pos<rows.length-1?`1px solid ${D.border}`:"none", background:u.id===highlightId?D.goldDim+"55":"transparent" }}>
          ...
          <div style={{ textAlign:"right" }}>
            <div style={{ fontSize:16, fontWeight:900, color:colorVsPar(u.vsPar) }}>{fmtVsPar(u.vsPar)}</div>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11, minWidth:pares.length*32+90 }}>
          <thead>
            <tr>
              ...
              <th style={{ padding:"4px 6px", color:D.gold, fontWeight:700, borderLeft:`1px solid ${D.border}` }}>vs Par</th>
              <th style={{ padding:"4px 6px", color:D.gold, fontWeight:700 }}>vs Par −HP</th>
            </tr>
            <tr>
          ...
          <tbody>
            {rows.map(u => (
              <tr key={u.id} style={{ borderTop:`1px solid ${D.border}` }}>
                ...
                <td style={{ textAlign:"center", padding:"5px 6px", fontWeight:900, color:D.gold }}>{u.brutoReal}</td>
                <td style={{ textAlign:"center", padding:"5px 6px", fontWeight:900, color:colorVsPar(u.vsPar), borderLeft:`1px solid ${D.border}` }}>{fmtVsPar(u.vsPar)}</td>
                <td style={{ textAlign:"center", padding:"5px 6px", fontWeight:900, color:colorVsPar(u.vsParHc) }}>{fmtVsPar(u.vsParHc)}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function TeamPlayView({ codigo, onExit }) {
  ...
  const irAHoyo = (nuevoHoyo) => { commitParSiFalta(); setHole(nuevoHoyo); };
          ...
          {tab === "marcar" && marcoA && (
            <Card>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginBottom:10 }}>
                <button onClick={() => irAHoyo(Math.max(0,hole-1))} disabled={hole===0} style={{ width:36,height:36,borderRadius:"50%",border:`1px solid ${D.border}`,background:"transparent",color:D.text,cursor:"pointer",fontSize:18,opacity:hole===0?0.3:1 }}>‹</button>
                <div style={{ textAlign:"center", minWidth:100 }}>
                  <div style={{ fontSize:20, fontWeight:900 }}>Hoyo {hole+1}</div>
                  <div style={{ fontSize:12, color:D.gold, fontWeight:700 }}>PAR {par}</div>
                </div>
                <button onClick={() => irAHoyo(Math.min(pares.length-1,hole+1))} disabled={hole===pares.length-1} style={{ width:36,height:36,borderRadius:"50%",border:`1px solid ${D.gold}`,background:D.goldDim,color:D.gold,cursor:"pointer",fontSize:18,opacity:hole===pares.length-1?0.3:1 }}>›</button>
              </div>
  ...
  if (screen==="unidades" && torneo) {
    ...
    return (
      <div style={appSt}>
        ...
        <div style={{ padding:"12px 12px" }}>
          <TabBar tabs={[{key:"unidades",label:"👤 Unidades"},{key:"grupos",label:"🔗 Grupos y códigos"},{key:"captura",label:"✏️ Capturar"},{key:"live",label:"🏆 En vivo"}]} active="unidades" onChange={setScreen} />
          <Card>
  ...
  if (screen==="grupos" && torneo) {
    ...
    return (
      <div style={appSt}>
        <Header title={torneo.nombre} />
        <div style={{ padding:"12px 12px" }}>
          <TabBar tabs={[{key:"unidades",label:"👤 Unidades"},{key:"grupos",label:"🔗 Grupos y códigos"},{key:"captura",label:"✏️ Capturar"},{key:"live",label:"🏆 En vivo"}]} active="grupos" onChange={setScreen} />
  ...
  if ((screen==="live" || screen==="resultados") && torneo) {
    return (
      <div style={appSt}>
        <Header title={torneo.nombre} />
        <div style={{ padding:"12px 12px" }}>
          <TabBar tabs={[{key:"unidades",label:"👤 Unidades"},{key:"grupos",label:"🔗 Grupos y códigos"},{key:"captura",label:"✏️ Capturar"},{key:"live",label:"🏆 En vivo"}]} active="live" onChange={setScreen} />
  ...
  if (screen==="captura" && torneo) {
    ...
    return (
      <div style={appSt}>
        <Header title={torneo.nombre} />
        <div style={{ padding:"12px 12px" }}>
          <TabBar tabs={[{key:"unidades",label:"👤 Unidades"},{key:"grupos",label:"🔗 Grupos y códigos"},{key:"captura",label:"✏️ Capturar"},{key:"live",label:"🏆 En vivo"}]} active="captura" onChange={setScreen} />
          <Card>
