...
function FranjaPatrocinadores({ torneo, big }) {
  ...
  return (
    <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"center", gap:big?28:16, padding:big?"16px 12px":"10px 8px", opacity:0.85 }}>
      ...
      {items.map((p, i) => (
        <img key={p.id||i} src={p.urlResuelta} alt={p.nombre || `Patrocinador ${i+1}`} style={{ height:big?46:22, width:"auto", maxWidth:big?220:130, objectFit:"contain", flexShrink:0 }} onError={e=>{e.target.style.display="none";}} />
      ))}
    </div>
  );
}
