...
// ─── FIREBASE ──────────────────────────────────────
// Mismo proyecto que H19 Golf (comparte el directorio de jugadores y sus
// handicaps). Si prefieres una base de datos separada, cambia este config.
...
function calcTotales(unidad, pares) {
  ...
  const brutoEstimado = pares.reduce((acc, par, i) => acc + (scores[i] ?? par ?? 0), 0);
  const neto = brutoEstimado - (unidad.hcAplicado || 0);
  return { jugados, brutoReal, brutoEstimado, neto };
}
      ...
      {rows.map((u, pos) => (
        <div key={u.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:pos<rows.length-1?`1px solid ${D.border}`:"none", background:u.id===highlightId?D.goldDim+"55":"transparent" }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ fontSize:10, color:D.textSub }}>HC {u.hcAplicado} · {u.jugados}/{torneo.pares.length} hoyos</div>
          </div>
          <div style={{ textAlign:"right" }}>
            <div style={{ fontSize:17, fontWeight:900, color:pos===0?D.gold:D.text }}>{u.neto}</div>
            <div style={{ fontSize:9, color:D.textSub }}>neto</div>
          </div>
        </div>
      ))}
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11, minWidth:pares.length*32+90 }}>
          <thead>
            <tr>
              ...
              <th style={{ padding:"4px 6px", color:D.gold, fontWeight:700 }}>Neto</th>
            </tr>
            <tr>
          ...
          <tbody>
            {rows.map(u => (
              <tr key={u.id} style={{ borderTop:`1px solid ${D.border}` }}>
                <td style={{ padding:"5px 6px", fontWeight:600, position:"sticky", left:0, background:D.surface, whiteSpace:"nowrap" }}>{u.nombre}</td>
                {pares.map((par, h) => {
                  const s = (u.scores||[])[h];
...
function TeamPlayView({ codigo, onExit }) {
  ...
  const miScore = (meMarca?.scores || [])[hole];
    ...
    <div style={appStyle}>
      ...
      {torneo.status === "finalizada" ? (
        ...
        <div style={{ padding:"0 12px 32px" }}>
          ...
          {tab === "marcar" && marcoA && (
            <Card>
              <SLabel>Anotas para: {marcoA.nombre}</SLabel>
          ...
          {tab === "mio" && (
            <Card>
              <SLabel>Tu score {meMarca ? `· anotado por ${meMarca.nombre}` : ""}</SLabel>
...
function AdminTorneoApp({ onExit }) {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(dirRef, snap => {
      if (snap.exists()) { const data = snap.val(); setDir(data.players || []); setNid(data.nextId || 6); }
      else {
  ...
  const saveDir = (newPlayers, newNidVal) => set(ref(db, "directorio"), { players:newPlayers, nextId:newNidVal||nid });
  const addPlayer = () => {
